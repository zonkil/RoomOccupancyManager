...
class DefaultRoomOccupancyCalculationV2ServiceTest extends Specification {

    @Subject
    DefaultRoomOccupancyCalculationV2Service defaultRoomOccupancyCalculationV2Service

    def "testCalculationRomOccupancy"() {
        given:
        def rooms = AvailableRooms.of(numPremium, numEconomy)
        def guestService = new TestGuestService(allGuests)
        defaultRoomOccupancyCalculationV2Service = new DefaultRoomOccupancyCalculationV2Service(guestService)
        when:
        def calculation = defaultRoomOccupancyCalculationV2Service.calculateRoomOccupancy(rooms)
        then:
        calculation != null
        with(calculation) {
