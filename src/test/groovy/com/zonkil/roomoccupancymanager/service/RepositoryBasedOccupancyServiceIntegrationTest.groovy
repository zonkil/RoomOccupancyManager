...
class RepositoryBasedOccupancyServiceIntegrationTest extends Specification {

    @Subject
    GuestsRoomOccupancyService guestsRoomOccupancyService


    def "shouldCalculateRoomOccupancy"() {
        given:
        def dividedGuests = new DefaultGuestsFactory(new EmptyPremiumRoomUpgradeService(100.0)).createGuests(guests)
        def guestService = new GuestsGuestService(dividedGuests)
        guestsRoomOccupancyService = new GuestsRoomOccupancyService(guestService)

        when:
        def calculation = guestsRoomOccupancyService.calculateRoomOccupancy(AvailableRooms.of(numPremium, numEconomy))
